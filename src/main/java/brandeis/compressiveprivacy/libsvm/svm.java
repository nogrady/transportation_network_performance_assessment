...
class Solver {
	...
	void Solve(int l, QMatrix Q, double[] p_, byte[] y_, double[] alpha_, double Cp, double Cn, double eps,
		...
		// calculate objective value
		{
			...
			// TODO
			/*
			 * double sum_dis=0; for(double a_d:alpha) { sum_dis+=a_d; } double
			 * w_abs=Math.sqrt(2*(sum_dis+si.obj)); System.out.println("w_abs: "+w_abs);
			 */
		}
...
public class svm {
	...
	public static int svm_cross_validation(svm_problem prob, svm_parameter param, int nr_fold, double[] target) {
		int nSV_out = 0;
		...
		for (i = 0; i < nr_fold; i++) {
			...
			for (int ii = 0; ii < submodel.nSV.length; ii++) {
				nSV_out = nSV_out + submodel.nSV[ii];
			}
	...
	public static double svm_predict(svm_model model, svm_node[] x) {
		...
		if (model.param.svm_type == svm_parameter.ONE_CLASS || model.param.svm_type == svm_parameter.EPSILON_SVR
			...
			// TODO
//			System.out.println("dec_values: "+dec_values[0]);
		}
